...
if (!isset($_SESSION['form_time']) || (time() - $_SESSION['form_time']) < 3) {
	echo json_encode(["success" => false, "message" => "El formulario se envió demasiado rápido."]);// <--------------------------------------
	exit;
}
...
if (!isset($_SESSION['captcha_answer']) || 
	...
	echo json_encode(["success" => false, "message" => "Respuesta de CAPTCHA incorrecta."]);// <----------------------------------------------
	exit;
}
...
if (!empty($_FILES['files']['name'][0])) {
	...
	if (count($_FILES['files']['name']) > $maxFiles) {
		echo json_encode(["success" => false, "message" => "Demasiados archivos."]);// <---------------------------------------------
		exit;
	}
