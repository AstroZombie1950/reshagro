...
if (!isset($_SESSION['form_time']) || (time() - $_SESSION['form_time']) < 3) {
	echo json_encode(["success" => false, "message" => "Le formulaire a été envoyé trop rapidement."]);// <--------------------------------------
	exit;
}
...
if (!isset($_SESSION['captcha_answer']) || 
	...
	echo json_encode(["success" => false, "message" => "Réponse CAPTCHA incorrecte."]);// <----------------------------------------------
	exit;
}
...
if (!empty($_FILES['files']['name'][0])) {
	...
	if (count($_FILES['files']['name']) > $maxFiles) {
		echo json_encode(["success" => false, "message" => "Trop de fichiers."]);// <---------------------------------------------
		exit;
	}
