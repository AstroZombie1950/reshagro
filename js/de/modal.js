document.addEventListener("DOMContentLoaded", function () {
	...
	form.addEventListener("submit", async function (e) {
		...
		if (files.length > maxFiles) {
			...
			showError("Maximal 10 Dateien erlaubt.");
			return;
		}
		...
		for (let file of files) {
			...
			if (!allowedExtensions.includes(ext) || file.size > maxFileSize) {
				setInvalid(fileInput);
				showError("Unzulässige Datei oder Dateigröße überschritten.");
				return;
			}
		}
		...
		try {
			const response = await fetch("/php/de/send.php", {
				method: "POST",
				body: formData
			});
			...
			if (result.success) {
				showSuccess("Ihre Anfrage wurde erfolgreich gesendet. Wir werden uns in Kürze mit Ihnen in Verbindung setzen.");
				form.reset();
				loadCaptcha();
			} else {
				showError(result.message || "Fehler beim Senden.");
			}
