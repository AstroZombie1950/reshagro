document.addEventListener("DOMContentLoaded", function () {
	...
	form.addEventListener("submit", async function (e) {
		...
		if (files.length > maxFiles) {
			...
			showError("Maksymalnie 10 plików.");// <--------------------------------------------------------------------------------
			return;
		}
		...
		for (let file of files) {
			...
			if (!allowedExtensions.includes(ext) || file.size > maxFileSize) {
				setInvalid(fileInput);
				showError("Nieprawidłowy plik lub przekroczony rozmiar.");// <---------------------------------------------------------
				return;
			}
		}
		...
		try {
			const response = await fetch("/php/pl/send.php", {
				method: "POST",
				body: formData
			});
			...
			if (result.success) {
				showSuccess("Twoje zgłoszenie zostało pomyślnie wysłane. Skontaktujemy się z Tobą wkrótce.");// <----------------------------
				form.reset();
				loadCaptcha();
			} else {
				showError(result.message || "Błąd wysyłki.");// <------------------------------------------------------------
			}
