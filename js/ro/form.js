	...
	form.addEventListener("submit", async function (e) {
		...
		for (let file of files) {
			...
			if (!allowedExtensions.includes(ext) || file.size > maxFileSize) {
				setInvalid(fileInput);
				showError("Fișier nevalid sau dimensiunea este depășită.");// <---------------------------------------------------------
				return;
			}
		}
		...
		try {
			const response = await fetch("/php/ro/send.php", {
				method: "POST",
				body: formData
			});
			...
			if (result.success) {
				showSuccess("Cererea a fost trimisă cu succes. Vă vom contacta în cel mai scurt timp.");// <----------------------------
			} else {
				showError(result.message || "Eroare la trimitere.");// <------------------------------------------------------------
			}
