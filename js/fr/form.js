document.addEventListener("DOMContentLoaded", function () {
	...
	form.addEventListener("submit", async function (e) {
		...
		if (files.length > maxFiles) {
			...
			showError("Maximum 10 fichiers autorisés.");// <--------------------------------------------------------------------------------
			return;
		}
		...
		for (let file of files) {
			...
			if (!allowedExtensions.includes(ext) || file.size > maxFileSize) {
				setInvalid(fileInput);
				showError("Fichier non valide ou taille dépassée.");// <---------------------------------------------------------
				return;
			}
		}
		...
		try {
			const response = await fetch("/php/fr/send.php", {
				method: "POST",
				body: formData
			});
			...
			if (result.success) {
				showSuccess("Votre demande a été envoyée avec succès. Nous vous contacterons prochainement.");// <----------------------------
				form.reset();
				loadCaptcha();
			} else {
				showError(result.message || "Erreur d’envoi.");// <------------------------------------------------------------
			}
