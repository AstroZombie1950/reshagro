document.addEventListener("DOMContentLoaded", function () {
	...
	form.addEventListener("submit", async function (e) {
		...
		if (files.length > maxFiles) {
			...
			showError("Maksimalno 10 fajlova.");// <--------------------------------------------------------------------------------
			return;
		}
		...
		for (let file of files) {
			...
			if (!allowedExtensions.includes(ext) || file.size > maxFileSize) {
				setInvalid(fileInput);
				showError("Nedozvoljen fajl ili je prekoračena veličina.");// <---------------------------------------------------------
				return;
			}
		}
		...
		try {
			const response = await fetch("/php/sr/send.php", {
				method: "POST",
				body: formData
			});
			...
			if (result.success) {
				showSuccess("Zahtev je uspešno poslat. Kontaktiraćemo vas u najkraćem roku.");// <----------------------------
			} else {
				showError(result.message || "Greška prilikom slanja.");// <------------------------------------------------------------
			}

		} catch (error) {
			submitBtn.classList.remove("is-loading");
			showError("Greška u konekciji.");// <--------------------------------------------------------------------------------
		}
	});
